...
c
c.....itype=1  TIME SERIES HEADER -- no channel location codes
c
      nwb(io)=4
      lhed(io)=30
      natt(io)=7
...
c
   30 if(itype.ne.3) goto 40
      ...
      iatt(2,7,io)=5
      iatt(3,7,io)=8
      attrem(8,io)='source colat'
      iatt(1,8,io)=11
      iatt(2,8,io)=11
      iatt(3,8,io)=5
      attrem(9,io)='source colong'
      iatt(1,9,io)=12
      iatt(2,9,io)=12
      iatt(3,9,io)=5
      attrem(10,io)='source depth'
      iatt(1,10,io)=13
      iatt(2,10,io)=13
      iatt(3,10,io)=5
      attrem(11,io)='source day'
      iatt(1,11,io)=15
      iatt(2,11,io)=15
      iatt(3,11,io)=1
      return
...
c
   70 if (itype.ne.7) goto 80
      ...
      iatt(2,4,io)=5
      iatt(3,4,io)=4
ccc     attrem(5,io)='yr,dy,hr,mn,sec '
      attrem(5,io)='disabled        '
      iatt(1,5,io)=4
      iatt(2,5,io)=4
      iatt(3,5,io)=0
ccc      attrem(6,io)='ditto but range '
      attrem(6,io)='disabled        '
      iatt(1,6,io)=4
      iatt(2,6,io)=4
      iatt(3,6,io)=0
      ...
      return
c
c  New (type 8) Differential travel time headers
c
   80 if(itype.ne.8) goto 100
      nwb(io)=0
      lhed(io)=65
      natt(io)=12
      attrem(1,io)='station name    '
      ...
      return
c
c.....itype=10 NEW TIME SERIES HEADER -- with channel location codes!!!
c
 100  if(itype.ne.10) goto 9999
      nwb(io)=4
      lhed(io)=100
      natt(io)=8
      attrem(1,io)='network code    '
      iatt(1,1,io)=2
      iatt(2,1,io)=2
      iatt(3,1,io)=0
      
      attrem(2,io)='station name    '
      iatt(1,2,io)=3
      iatt(2,2,io)=3
      iatt(3,2,io)=0
      
      attrem(3,io)='location code   '
      iatt(1,3,io)=4
      iatt(2,3,io)=4
      iatt(3,3,io)=0

      attrem(4,io)='channel name    '
      iatt(1,4,io)=5
      iatt(2,4,io)=5
      iatt(3,4,io)=0
      ...
      attrem(7,io)='yr,dy,hr,mn,sec '
      iatt(1,7,io)=6
      iatt(2,7,io)=6
      iatt(3,7,io)=7
      
      attrem(8,io)='ditto but range '
      iatt(1,8,io)=6
      iatt(2,8,io)=6
      iatt(3,8,io)=8
      return

 9999 PRINT*,'CURRENTLY ONLY HEADER TYPE itype=1,3,4,5,6,7,8,10',
     *       ' ARE DEFINED'
      return
      end
c
...
c
c Obviously this is just a simplistic call to wrhdr, to maintain 
c compatability with codes that call prhdr.
c
      dimension head(*)
      iounit = 6
      call wrhdr(iounit,nfl,head,itype)
      return
      end
c
c--------------------------------------------------------------------
...
c
     ...
     +    iys8,ids8,ihs8,ims8,sss8,stim,toffset,npts8,                          
     +    pnam,ptim,amp,tstar,pol,qual,ctim,ecor,modid,tlat,tlon,   
     +    tdep,tcode,extra(11)
c end of type 8 definitions
c
c
c for type = 10
      common/myhd10/nscan10,nnet10,nsta10,nloc10,nchn10,
     &              iy10,id10,ih10,im10,sec10,dt10,spare10(89)
c
c now do all the equivalences
c
      equivalence (nscan,hed1),(mfreq,hed4),(nots,hed5)
      equivalence (nhed,hed6),(nscan7,hed7),(nscan8,head8)
      equivalence (nscan10,hed10)
c
c
      if(itype.ne.1) goto 30
c
...
c
  ...
  901 format(i5,2x,a4,2x,a4,2x,a4,i6,i4,':',i2,':',i2,':',f6.3,2X,
     &       f8.3,i8)
      return
   30 if(itype.ne.2.and.itype.ne.3) goto 40
c
c.....itype=3  TIME SERIES HEADER WITH SOURCE INFO INCLUDED
c
      do 3 i=1,18
    3 hed1(i)=head(i)
      write(iout,903) nfl,nsta,nchn,ntyp,iy,id,ih,im,sec,dt,nscan,
     +                slat,slon,sdep,jys,jds,jhs,jms,ssec
  903 format(i5,2x,a4,2x,a4,2x,a4,i6,i4,':',i2,':',i2,':',f6.3,2x,
     &       f8.3,i8,
     & f6.1,2x,f6.1,2x,f4.0,i6,i4,':',i2,':',i2,':',f6.3)
      return
   40 if(itype.ne.4) goto 50                       
c
c.....itype=4  SPECTRA OF EVENTFILES
...
c
  ...
  907 format(i5,2x,a4,2x,a4,2x,i4,1x,i3,1x,i2,':',i2,':',f5.2,2x,i4,2x,
   ...
   80 if(itype.ne.8) goto 100
c
c.....itype=8 differential travel time headers (phase-pair specified)
c
      do 8 i=1,65
    8 head8(i)=head(i)
 ...
 9088 format(i5,1x,a4,1x,a4,i5,i4,':',i2,':',i2,':',f6.3,
  ...
  100 if(itype.ne.10) goto 9999
c
c.....itype=10  TIME SERIES HEADER, WITH NETWORK AND LOCATION CODES
c
      do 10 i=1,11
   10   hed10(i)=head(i)
      isec  = int(sec10)
      ifrac = nint( 10000.0 * (sec10 - float(isec)) )
      write(iout,9010) nfl,nnet10,nsta10,nloc10,nchn10,iy10,id10,
     &                 ih10,im10,isec,ifrac,dt10,nscan10
      
 9010 format(i5,2x,2(a2,'/',a4,2x),i4,',',i3.3,',',
     &       i2.2,':',i2.2,':',i2.2,".",i4.4,2X,f9.4,2x,i9)
      return
