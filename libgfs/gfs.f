...
c*** append
      ...
      call gfs_rwdata(io,a,nfil(io),1,head(1),'w',ierr)
...
c*** read
    5 if(kfl.gt.nfil(io)) goto 6
      ...
      call gfs_rwdata(io,a,kfl,1,head(1),'r',ierr)
      if(iret+ierr.eq.0) return
    6 print *,'unable to read entry ',kfl
      return
c*** overwrite
   10 call gfs_rwdir(io,head,kfl,'w',iret)
      call gfs_rwdata(io,a,kfl,1,head(1),'w',ierr)
      if(iret+ierr.eq.0)return
...
c*** interactively gets search parameters
  ...
  100 format(10a4)
   ...
   10 nlist=nlist+1
      list(nlist)=is
      do 15 i=1,10
   15   flist(i,nlist)=reel(i)
      goto 20
   30 icanc=iabs(is)
      if(icanc.gt.nlist) goto 20
      if(icanc.lt.nlist) then
        do 35 j=icanc+1,nlist
          list(j-1)=list(j)
          do 35 i=1,10
   35       flist(i,j-1)=flist(i,j)
      end if
      nlist=nlist-1
   40 call gfs_prlist(io)
      ...
      subroutine GFS_NODEF(io)
c*** this simply sets default search parameters. The effect is
c the same as calling gfs_sdef and entering a "0" right away
c (i.e. not searching on any paramters).
c
      common/srXXXX/nlist,list(40),flist(10,40)
      common/atXXXX/natt(10),iatt(3,20,10),attrem(20,10)
      character*16 attrem
  ...
  130   format(i5,5x,a16,5x,2g14.6)
   ...
   10   continue
     ...
     +             'time    (range) '/
      ...
      do 10 i=1,natt(io)
        ipr=iatt(3,i,io)+1
   10   print 900,i,attrem(i,io),comment(ipr)
...
c  modified so that this takes all entries if nlist = 0
   ...
   15 if(kfl.gt.nfil(io)) goto 500
      ...
      match=0
      do 20 i=1,nlist
   20   match=match+matchX(io,i,hed)
...
c  returns 1 if a match,otherwise returns 0
   ...
   22 if(iconvX(hed(i1)).ne.iconvX(flist(1,k))) matchX=0
    ...
    6 dh=dconvX(hed(i1))
      ...
      return
    7 call dattoe(dh,hed(i1),hed(i1+1),hed(i1+2),hed(i1+3),hed(i1+4))
      if(dh.ne.dconvX(flist(1,k))) matchX=0
      return
    8 call dattoe(dh,hed(i1),hed(i1+1),hed(i1+2),hed(i1+3),hed(i1+4))
      if(dh.lt.dconvX(flist(1,k)).or.dh.gt.dconvX(flist(3,k))) matchX=0
      return
      end
...
c update directory header
      ...
      nentry=jentry
      ntype=jtype
      call gpodiscb(lid(io),1,0)
      call gwrdiscb(lid(io),nentry,l4,istat)
      ierr=0
...
c*** get number of points in header
      ...
      call gpodiscb(lid(io),1,nb)
      call grddiscb(lid(io),npts,4,ierr)
      if(ierr.ne.4) return
...
c*** copy npts of data to a temp file
      call ggetfil(3,lun,'ts.temp',ierr)
      if(ierr.ne.0) return
      nbc=npts*nwb(io)
      j1=0
    5   nwr=min0(40000,nbc)
        call gpodiscb(lid(io+10),1,j1)
        call grddiscb(lid(io+10),dum,nwr,ierr)
        if(ierr.lt.0) goto 500
        call gpodiscb(lun,1,j1)
...
c  iret=-3 fatal error while opening new file,any mess is cleaned up.
    ...
    5     plen(i)=0
      ...
      if(.not.there) then
        ...
        ierr=system('mkdir '//pname(1:nlen))
        if(ierr.ne.0) goto 500
      end if
