      subroutine bdf2gfs(io_bdf,io_gfs,itype,indx_end)
...
c 
      character label*4,string*80,nsta*4,nchn*4,ntyp*4
      dimension head(30)
...
c use this syntax to catch both "NET " and "NETW"
      ...
      elseif (label.eq.'TIME') then
        read(string,'(i4,1x,i3,1x,i2,1x,i2,1x,f6.3)',err=5) 
     &       iy,id,ih,im,ss
      ...
      elseif (label.eq.'DATA' .and. nscan.gt.0 ) then
        p1 = malloc(4*nscan)
        if (p1.eq.0) then
          write(*,*) 'ERROR - unable to malloc in bdf2gfs'
          return
        endif
        npts = nscan
...
c
        if ( dt.ne.0.0 ) then
          ...
          elseif (npts.ne.nscan .and. npts.gt.0) then
            ...
            write(*,*) 'ERROR -- no data read, skipping time series :'
...
c   
      ...
      dimension head1(30), head2(30)
c
      common/myhed1/nscan,nsta,nchn,ntyp,iy,id,ih,im,ss,dt,spare(20)
      common/myhed2/nscan2,nsta2,nchn2,ntyp2,spare2(26)
...
c
      ...
      real*8 time_ref
      common /appinfo/indx_ref(maxseis),nsta_ref(maxseis),
     &                nchn_ref(maxseis),ntyp_ref(maxseis),
...
c
      ...
      character*4 nsta_ref, nchn_ref, ntyp_ref
      common /appinfo/indx_ref(maxseis),nsta_ref(maxseis),
     &                nchn_ref(maxseis),ntyp_ref(maxseis),
     &                dt_ref(maxseis),time_ref(maxseis),
     &                nscan_ref(maxseis),iseis
c      
      ...
      do 10 i=iseis,1,-1
        ...
        if (nsta  .eq. nsta_ref(i) .and.
     &      nchn  .eq. nchn_ref(i) .and.
     &      ntyp  .eq. ntyp_ref(i) .and.
     &      dt    .eq. dt_ref(i)   .and.
...
c--------------------------------------------------------------
c     
      subroutine bdf2gfs_nostring(io_bdf,io_gfs,itype,indx_end)
c
c Converts ASL "bdf" files to GFS format.
c
c This version does not attempt to "string" the output file.
c Each individual data segment in the input BDF file is written as
c a single GFS time series. The only change relative to subroutine
c "bdf2gfs" is that this one calls "dump_gfs_nostr" instead 
c of "dump_gfs")
c
c input:
c  io_bdf = unit number of input BDF file (can be standard in)
...
c
        if ( dt.ne.0.0 ) then
          ...
          elseif (npts.ne.nscan .and. npts.gt.0) then
            write(*,*) 'WARNING -- incorrect number of samples:'
            write(*,*) '  wanted: ',nscan,'  got: ',npts
            write(*,*) '  writing this time series anyway'
            nscan = npts
            call dump_gfs_nostr(io_gfs,head,rdata,indx_end,itype,iverb)
          else
            write(*,*) 'ERROR -- no data read, skipping time series :'
...
c--------------------------------------------------------------
c
      subroutine bdf2gfs_splice(io_bdf,io_gfs,itype,indx_end)
c
c Converts ASL "bdf" files to GFS format.
c
c This version just like the primary "bdf2gfs" except it calls
c dump_gfs_splice, which will splice on segments, eliminating 
c duplication.
c
c input:
c  io_bdf = unit number of input BDF file (can be standard in)
...
c (if we return, note we don't update header time)
     ...
     &                         rdata,ncomp,ierr)
              if (ierr.ne.0) then
                ...
                write(0,20) ntyp,nsta,nchn,iy,id,ih,im,ss
...
c      
 ...
 20   format(a2,1x,2(a4,1x),i4,',',i3.3,',',2(i2.2,':'),f6.3)
      return
      end
c
c-------------------------------------------------------------
