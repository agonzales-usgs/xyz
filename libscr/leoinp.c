  ...
  do {
    ...
    switch( report.type ) {
      ...
      case SelectionRequest:
		error("received unexpected Selection event.");
		done=0;
		break;
      case MappingNotify:  
		error("warning: keyboard/mouse remapped.");  
		done=0;
		break;
      case ClientMessage:  
		sprintf(s,"received message: \"%20s\".",report.xclient.data.b);
		error(s);
		done=0;
		break;
      case NoExpose: 
		error("Unexpected Noexpose event.");  
		done=0;
		break;
      case GraphicsExpose:  
		error("Unexpected GraphicsExpose event.");  
		done=0;
		break;
...
void leventnowait_(main, x, y, mod, typ)
/* Waits for an event of the selected types, and then returns
information about it.  Main is the main thingy returned.  x, and y are
the position of the pointer when the button is pressed.  mod is the
state of the modifier keys when the event occured */
long *main, *x, *y, *mod, *typ;
{
  ...
  *main=0;
  ...
  if (XCheckWindowEvent(display,win,user_event_mask,&report)) {
    done=1;  /* Default. */
    switch( report.type ) {
      case ButtonPress: /* Trickle through to ButtonRelease */
      case ButtonRelease: 
